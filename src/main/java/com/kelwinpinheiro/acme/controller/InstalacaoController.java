package com.kelwinpinheiro.acme.controller;

import com.kelwinpinheiro.acme.domain.Cliente;
import com.kelwinpinheiro.acme.domain.Instalacao;
import com.kelwinpinheiro.acme.exception.RecursoNotFoundException;
import com.kelwinpinheiro.acme.repositor.ClienteRepository;
import com.kelwinpinheiro.acme.repositor.InstalacaoRepository;
...
public class InstalacaoController {
	...
	public List<Instalacao> getInstalacaoPorCPF(@PathVariable String cpf)
	{
		...
		try {
			cliente = Optional.ofNullable(clienteRepository.findByCpf(cpf));
	...
	public ResponseEntity<Object> cadastrarInstalacao(@RequestBody Instalacao instalacao)
	{
		...
		URI location = null;
		try {
			cliente = Optional.ofNullable(clienteRepository.findByCpf(instalacao.getCliente().getCpf()));
